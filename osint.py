...
def open_results_folder():
    ...
    if os.path.exists(results_dir):
        # Ouvrir le dossier en fonction du système d'exploitation
        ...
        result_label.config(text="Le dossier n'existe pas.")
...
def execute_scripts():
    ...
    if script1_var.get():
        subprocess.run(['python3', 'scriptshodan.py', domain, results_dir, shodanapikey])

    if script2_var.get():
        subprocess.run(['python3', 'scriptdnscan.py', domain, results_dir])

    if script3_var.get():
        subprocess.run(['python3', 'scripturlscan.py', domain, results_dir, urlscanapikey])

    if script4_var.get():
        subprocess.run(['python3', 'scriptharvester.py', domain, results_dir])

    if script5_var.get():
        subprocess.run(['python3', 'dorks.py', domain, results_dir, googleapikey, searchidengine])
...
# Bouton pour ouvrir le dossier des résultats
open_folder_button = tk.Button(mainapp, text="Ouvrir le dossier", command=open_results_folder, font=("Arial", 12), bg="#F8F9FA", fg="#2B2D42", padx=20, pady=10)
